...
const jobsQueue = new Queue('jobSingle', {
    redis: {
        host: config.REDIS.HOST, // Redis server host
        port: config.REDIS.PORT,        // Redis server port
    },
...
//         const min = currentDate.getMinutes() + 10;
...
//     }
//     schedulerQueue.process(async (job: Job, done: any) => {
...
//                 for (let i in getFCM) {
//                     sendNotification({ title: 'New Task Alert for ' + job.data.name, body: "Assign Janitor to the task" }, null, getFCM[i].fcm_token);
//                 }
//             }
//             done();
//         } catch (error) {
...
const createJobs = async () => {
    ...
    for (let i = 0; i < getTasks.auto_tasks_allocations.length; i++) {
        ...
        currentDate.setHours(taskTime.getHours());
        currentDate.setMinutes(taskTime.getMinutes());
        // const options: any = { timeZone: 'Asia/Kolkata', timeZoneName: 'short' };
        // console.log(new Date(currentDate).toLocaleTimeString('en-IN', options));
        const cronExpression = generateCronExpression(new Date(currentDate).toISOString());
        const name = generateRandomId(8);
        jobs.push({ name: name, cron: cronExpression, jobData: getTasks.auto_tasks_allocations[i] });
    }
    return jobs;
}

const processJobs = async () => {
    let jobs = await createJobs();
    for (const jobDatas of jobs) {
        const { name, cron, jobData } = jobDatas;
        // Add the job to the queue with the defined schedule
        await jobsQueue.add(name, { data: jobData }, { removeOnComplete: true, removeOnFail: true, repeat: { cron } });
        // console.log(`Scheduled job ${name} with cron ${cron}`);
        jobsQueue.process(name, async singleJob => {
            if (!singleJob.isDelayed() || !singleJob.isPaused()) {
                try {
                     let getTask = await taskAllocationModel.getTaskByByJanitorId(singleJob.data.data.janitor_id, singleJob.data.data.task_template.task_id);
                     const makeStartDate = makeCurrentDateWithTime(singleJob.data.data.start_time);
                     const makeEndDate = makeCurrentDateWithTime(singleJob.data.data.end_time)
                     if (!getTask.length) {
                         await taskAllocationModel.create({
                             facility_id: singleJob.data.data.facility_id,
                             template_id: singleJob.data.data.task_template.task_id,
                             status: 1,
                             start_time: makeStartDate,
                             end_time: makeEndDate,
                             request_type: 'Regular',
                             initial_janitor_id: singleJob.data.data.janitor_id
                         });
                         //get fcm for supervisor
                         const getFCM = (await new UserModel().getTokensForSupervisor(singleJob.data.data.cluster_id)).rows;
                         for (let i in getFCM) {
                             sendNotification({ title: 'New Task Alert for ' + singleJob.data.data.name, body: "Assign Janitor to the task" }, null, getFCM[i].fcm_token, getFCM[i].id);
                         }
                    }
                 } catch (error) {
...
schedulerQueue.add('main-job-creation', null, {
...
schedulerQueue.process('main-job-creation', async job => {
    console.log(`Processing main job ${job.id} with name ${job.name}`);
